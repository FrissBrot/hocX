...
class ProtocolService:
    ...
    def _add_virtual_dates(
        ...
        documents) into rank counts already computed from real DB rows. Every extra_date must
    ...
    def _renumber_later_siblings(
        ...
        *,
        ...
        for sibling in siblings:
            ...
            counts = self._add_virtual_dates(
                counts,
                extra_dates=[protocol_date],
    ...
    def preview_title(
        ...
        *,
        ...
        Read-only preview: counts reflect protocols that exist right now plus extra_dates (the
        word-import queue passes the protocol_date of every other still-open, not-yet-committed
        document for the same template here - without it, several documents queued together in
        one batch would all preview with the identical number, since none of them are real
        Protocol rows _sequence_counts can see yet). The final title generated by
        create_from_template() may still differ slightly if queue items get committed in a
        different order than previewed, or a still-open sibling gets edited/deleted meanwhile."""
        ...
        if extra_dates:
            counts = self._add_virtual_dates(
                counts, extra_dates=extra_dates, target_date=protocol_date, reset_month=reset_month, reset_day=reset_day
            )
