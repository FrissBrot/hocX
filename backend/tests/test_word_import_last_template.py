...
from app.schemas.word_import import PublicWordImportLastTemplate
...
def test_set_and_get_roundtrip(db):
    ...
    set_result = set_last_word_import_template(PublicWordImportLastTemplate(template_id=template.public_id), db=db, user=user)
    assert set_result.template_id == template.public_id

    get_result = get_last_word_import_template(db=db, user=user)
    assert get_result.template_id == template.public_id
...
def test_set_rejects_template_from_other_tenant(db):
    ...
    with pytest.raises(HTTPException) as exc_info:
        set_last_word_import_template(PublicWordImportLastTemplate(template_id=other_template.public_id), db=db, user=user)
...
def test_set_none_clears_preference(db):
    ...
    template = make_template(db, tenant.id)
    user = make_current_user(tenant.id)
    set_last_word_import_template(PublicWordImportLastTemplate(template_id=template.public_id), db=db, user=user)

    result = set_last_word_import_template(PublicWordImportLastTemplate(template_id=None), db=db, user=user)
