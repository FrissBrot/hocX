...
score_face_quality, write_face_quality_score, finish_job) are monkeypatched, so these
don't need a database or the real YuNet model."""
...
def test_process_job_marks_an_unreadable_file_as_analyzed_with_no_score(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(worker, "write_face_quality_score", lambda engine, file_id, score: written.append((file_id, score)))
