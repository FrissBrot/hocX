...
async function authenticate(email: string, file: string) {
  ...
  const response = await context.post("/api/login", { data: { email, password: process.env.E2E_USER_PASSWORD ?? "ChangeMe123!" } });
...
setup("creates reproducible role and tenant sessions", async () => {
  ...
  const session = await (await tenantContext.get("/api/session")).json();
  ...
  const selected = await tenantContext.post(`/api/select-tenant/${secondTenant.tenant_id}`);
