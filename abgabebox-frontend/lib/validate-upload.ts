...
export function validateUploadFiles(
  selected: File[],
  { maxFiles, allowedFileTypes, maxFileSizeMb }: { maxFiles: number; allowedFileTypes: string[]; maxFileSizeMb: number }
): UploadValidationResult {
  if (selected.length > maxFiles) {
    return { ok: false, error: `Maximal ${maxFiles} Datei(en) erlaubt` };
  }
